...
{
    public sealed class MoveDistanceValueStrategy : FloatValueBaseStrategy
    {
        ...
        public override bool TryGet(GameEntity entity, out float value)
        {
            ...
            if (!target.hasMoveDistance)
            {
                value = default;
                if (Logging) LogErrorNotFound("MoveDistanceComponent", (nameof(entity), entity), (nameof(target), target));
                return false;
            }

            value = target.moveDistance.Value;
            return true;
        }
    }
}